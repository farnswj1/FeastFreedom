...
export class ProvidersService {
  ...
  getKitchen(): Observable<Kitchen[]> {
    return this.http.get<Kitchen[]>(this._url1)
      .pipe(catchError(this.errorHandler));
  }

  postKitchen(KitchenData: any): Observable<Kitchen[]> {
    return this.http.post<Kitchen[]>(this._url, KitchenData)
      .pipe(catchError(this.errorHandler));
  }

  getKitchenUser(): Observable<KitchenUser[]> {
    return this.http.get<KitchenUser[]>(this._url)
      .pipe(catchError(this.errorHandler));
  }

  postKitchenUser(CredentialData: any): Observable<KitchenUser[]> {
    return this.http.post<KitchenUser[]>(this._url2, CredentialData)
      .pipe(catchError(this.errorHandler));
